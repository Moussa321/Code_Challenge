...
const validatePhoneNumber = async (req, res) => {
  if (!req.query.phoneNumber)
    res.status(404).json({ mssg: "Phone Number Input Not Valid" });

  // Make a request to the NumVerify API
  axios
    .get(
      `http://apilayer.net/api/validate?access_key=${process.env.PHONENUMBER_API_KEY}&number=${req.query.phoneNumber}&format=1`
    )
    .then((response) => {
