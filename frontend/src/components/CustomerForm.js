...
const CustomerForm = (props) => {
  ...
  const [address, setAddress] = useState(
    props.update ? props.customer.address : ""
  );
  const [mobileNumber, setMobileNumber] = useState(
    props.update ? props.customer.mobileNumber : ""
  );
  ...
  return (
    <form className="create" onSubmit={handleSubmit}>
      ...
      />
      {props.update && (
        <button className="error-button" onClick={() => props.setIsOpen(false)}>
          Cancel
        </button>
      )}
      ...
      {error && (
        <div className="error">
          {error} {emptyFields?.length > 0 ? ":" : null}{" "}
